...
import { BulletCardConfig } from '@/types';

interface BulletCardDisplayProps {
  config: BulletCardConfig;
  onEdit: () => void;
  onDelete: () => void;
}

export default function BulletCardDisplay({ config, onEdit, onDelete }: BulletCardDisplayProps) {
      ...
      <div className="p-6">
        <ul className="space-y-1">
          {config.items.map((item) => (
            ...
            >
              <span className="text-gray-400 mt-0.5">{getBulletChar(item.indent)}</span>
              <span>{item.text}</span>
            </li>
          ))}
        </ul>
